...
@Slf4j
@Component
public class Prim08 implements InputMessageHandler {
    private UserDataCache userDataCache;
    private ReplyMessagesService messagesService;

    public Prim08(UserDataCache userDataCache,
    ...
    private SendMessage processUsersInput(Message inputMsg) {
        ...
        if (usersAnswer.equals("1")) {
            replyToUser = messagesService.getReplyMessage(chatId, "reply.prim081");
            userDataCache.setUsersCurrentBotState(userId, BotState.SHOW_HI_TAG);
        }
        if (usersAnswer.equals("2")) {
            replyToUser = messagesService.getReplyMessage(chatId, "reply.prim082");
            userDataCache.setUsersCurrentBotState(userId, BotState.SHOW_HI_TAG);
        }
